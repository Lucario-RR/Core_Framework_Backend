...
- Invitation registration is stored in `auth.registration_invite`; only invite code hashes are persisted, while backend-generated or frontend/admin-provided codes are returned once from the admin API with `codeSource`.
- Admin-created custom roles are stored in `iam.role`; per-user role expiry is stored on `iam.account_role.expires_at` and expired roles are ignored when authorizing requests. Deleting a non-protected role soft-deletes it and expires active assignments; `admin` and `user` cannot be deleted.
- Admin endpoints are permission-gated by `admin:*` scopes, so custom staff roles can manage selected settings or operations without receiving the full `admin` role.
- Admin-created users default to `awaiting_setup` with password rotation required; the frontend should use `user.security.setupRequired`, `requiredSetupActions`, and `recommendedSetupActions` after login, then call `POST /api/v1/me/setup/complete` when setup blockers are cleared.
- Users can read login and account activity history, including time, IP, user-agent, and device labels, from `GET /api/v1/me/activity`.
