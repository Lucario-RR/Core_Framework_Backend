...
use crate::{
    api::contracts::{
        AdminSystemSettingUpdateRequest, AdminUserBulkActionRequest, AdminUserCreateRequest, AdminUserListQuery,
        AdminUserUpdateRequest, SearchPaginationQuery, SessionBulkRevokeRequest,
    },
    auth,
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/admin/users/{account_id}", get(get_admin_user).patch(update_admin_user))
        .route("/admin/users/{account_id}/sessions", get(list_admin_user_sessions))
        .route("/admin/users/{account_id}/sessions/revoke-all", post(revoke_admin_user_sessions))
        .route("/admin/users/{account_id}/security-events", get(list_admin_user_security_events))
        .route("/admin/users/{account_id}/audit-logs", get(list_admin_user_audit_logs))
        .route("/admin/users/{account_id}/emails/{email_id}/verify", post(admin_verify_user_email))
        .route("/admin/users/{account_id}/emails/{email_id}/unverify", post(admin_unverify_user_email))
        .route("/admin/users/bulk-actions", post(bulk_admin_user_action))
        .route("/admin/settings", get(list_system_settings))
        .route("/admin/settings/{setting_key}", patch(update_system_setting))
}
...
async fn list_audit_logs(
    ...
    let (logs, next_cursor) = admin_service::list_audit_logs(&state, query.query.as_deref(), offset, limit).await?;
    Ok(Json(envelope_with_cursor(&context.request_id, logs, next_cursor)))
}
...
async fn list_security_events(
    ...
    let _ = admin_auth(&state, &headers).await?;
    let (offset, limit) = pagination(query.cursor.as_deref(), query.limit)?;
    let (events, next_cursor) = admin_service::list_security_events(&state, query.query.as_deref(), offset, limit).await?;
...
async fn list_admin_users(
    ...
    .await?;
    Ok(Json(envelope_with_cursor(&context.request_id, users, next_cursor)))
}
...
async fn create_admin_user(
    ...
    Ok((axum::http::StatusCode::CREATED, Json(envelope(&context.request_id, user))))
}
...
async fn update_admin_user(
    ...
    let auth_context = admin_auth(&state, &headers).await?;
    let user = admin_service::update_admin_user(&state, &auth_context, &context, account_id, request).await?;
    Ok(Json(envelope(&context.request_id, user)))
}
...
async fn list_admin_user_sessions(
    ...
    let _ = admin_auth(&state, &headers).await?;
    let (offset, limit) = pagination(query.cursor.as_deref(), query.limit)?;
    let (sessions, next_cursor) = admin_service::list_admin_user_sessions(&state, account_id, offset, limit).await?;
...
async fn revoke_admin_user_sessions(
    ...
    let acknowledgement = admin_service::revoke_admin_user_sessions(&state, &auth_context, &context, account_id, request).await?;
...
async fn list_admin_user_security_events(
    ...
    let _ = admin_auth(&state, &headers).await?;
    let (offset, limit) = pagination(query.cursor.as_deref(), query.limit)?;
    let (events, next_cursor) = admin_service::list_admin_user_security_events(&state, account_id, offset, limit).await?;
    Ok(Json(envelope_with_cursor(&context.request_id, events, next_cursor)))
}
...
async fn list_admin_user_audit_logs(
    ...
    let _ = admin_auth(&state, &headers).await?;
    let (offset, limit) = pagination(query.cursor.as_deref(), query.limit)?;
    let (logs, next_cursor) = admin_service::list_admin_user_audit_logs(&state, account_id, offset, limit).await?;
    Ok(Json(envelope_with_cursor(&context.request_id, logs, next_cursor)))
}
...
async fn bulk_admin_user_action(
    ...
    let auth_context = admin_auth(&state, &headers).await?;
    let acknowledgement = admin_service::bulk_admin_user_action(&state, &auth_context, &context, request).await?;
    Ok(Json(envelope(&context.request_id, acknowledgement)))
}
...
async fn update_system_setting(
    ...
    let auth_context = admin_auth(&state, &headers).await?;
    let setting = admin_service::update_system_setting(&state, &auth_context, &setting_key, request).await?;
    Ok(Json(envelope(&context.request_id, setting)))
}

async fn admin_auth(state: &AppState, headers: &HeaderMap) -> crate::error::AppResult<auth::AuthContext> {
